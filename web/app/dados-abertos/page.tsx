...
const DATASET_LABELS: Record<string, string> = {
  DATA_DICTIONARY: "Dicionário de campos",
  health_regions_current: "Indicadores atuais por Região de Saúde",
  health_region_temporal: "Evolução dos indicadores",
  health_region_changes: "Mudanças entre períodos",
  health_region_financing: "Recursos gerais de saúde",
  health_region_flow_summary: "Resumo dos fluxos de internação",
  hospitalization_flows_public: "Fluxos de internação entre regiões",
  municipality_health_region_crosswalk: "Municípios e suas Regiões de Saúde",
  territorial_intelligence: "Sinais territoriais",
  health_region_peers: "Regiões estruturalmente semelhantes",
};
  ...
  return (
    <div className="page-shell open-platform-page">
      <section className="intro open-platform-intro">
        <span className="eyebrow">Dados agregados para reutilização</span>
        <h1>Dados abertos</h1>
        ...
        <div className="download-grid">
          {distributions.map((file) => {
            ...
            return <article className="download-item" key={file.relative_path}>
              <div><span className="file-format">{file.relative_path.endsWith(".csv") ? "CSV" : "PARQUET"}</span><h3>{DATASET_LABELS[dataset] ?? "Conjunto de dados agregados"}</h3></div>
